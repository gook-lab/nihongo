...
> 이 문서는 데이터가 어디에 살고 어떻게 흐르는가를 다룬다.
...
`src/data/`의 파일들은 순수 TypeScript 상수다. 서버도 CMS도 없다.
...
- `conversations.ts`가 2,000줄을 넘어가면 편집 자체가 불안정해진다.
- 증분 추가가 안전하고 추적이 쉽다.

같은 패턴이 `words-ext.ts`, `reading-ext.ts`, `songs-ext.ts`, `conversations-ext2.ts`에 적용된다.

> 새 카테고리를 추가하면 `ConversationPage`의 `ICON_MAP`에 lucide 아이콘 이름을 등록해야 한다.

### 콘텐츠 중복 금지 — 테스트로 강제

| 대상 | 규칙 | 안 지키면 |
|---|---|---|
| 회화 | 카테고리 내 일본어 문장 중복 금지 | TTS 캐시 키(`text:voiceId`)를 공유해서 **미리받기 완료 판정이 왜곡**된다 (실제 사고: 확장 누적으로 중복 30건) |
| 단어 | 같은 단어(한자+히라가나+뜻)는 하나의 항목·하나의 레벨로만 | 사전 이중 노출 + **SRS 이중 추적** (실제 정리: N2/N1/N3 재수록 71건 제거) |

`src/data/conversations.test.ts`, `src/data/words.test.ts`의 무결성 테스트가 이를 잠근다.
확장 전에는 기존 데이터와 **기계 대조**한다.

> **제거한 콘텐츠 id는 재사용하지 않는다.** 진행도·즐겨찾기·SRS가 엉뚱한 항목에 붙는다.

### 일본어 reading은 단어별로 띄어쓴다

```ts
// ✅ reading: 'ほんを かいます。'
// ❌ reading: 'ほんをかいます。'   ← hiraganaToRomaji가 음절을 구분하지 못한다
```
...
순수 함수다. 상태는 `WordSrsState { wordId, ease, intervalDays, dueDate, reviewCount, correctCount, wrongCount, lastReviewedAt }`.

- `ease`: 기본 2.5, 하한 1.3, 상한 2.5
- 정답: ease +0.1(상한 클램프), 간격 `0 → 1일 → 3일 → round(직전 × ease)`
- 오답: ease 하락 + 간격 초기화

순수 함수라 단위 테스트가 쉽고, UI/저장소와 독립적이다.
...
**왜 4개인가**: 바뀌는 빈도가 완전히 다르다. 학습 한 번에 `state`와 `srs`는 매번 바뀌지만
`profile`은 거의 안 바뀐다. 한 문서에 몰아넣으면 쓰기 비용과 충돌이 같이 커진다.

운영 규칙:

- **카테고리별 dirty flag → 1.5초 디바운스** 저장. 매 상호작용마다 쓰지 않는다.
- **`stripUndefined()` 필수** — Firestore가 `undefined`를 거부한다.
- 탈퇴 시 전체 카테고리 + 부모 user doc까지 삭제한다.
...
캐시 계층이 실패해도 **무해하게** 아래 계층으로 떨어진다 — IndexedDB를 못 쓰는
브라우저에서도 앱이 죽지 않는다.

회화 화면의 `PrefetchAudioButton`이 카테고리 오디오를 미리 받아두면
그 이후로는 네트워크 없이 발음이 재생된다.
...
데이터 초기화는 메인 키만 제거 + (선택) Firestore 카테고리 삭제 → `window.location.reload()`.

---

## 6. 에러 처리 — 사용자와 Sentry에 동시에

이 프로젝트의 모든 `catch`는 **사용자 피드백 + Sentry 보고**를 함께 한다.
...
- `ErrorBoundary`는 react-error-boundary 기반 — `<ErrorBoundary boundaryName="xxx">`
- **사용자 취소는 보고하지 않는다** — 팝업 닫기(`popup-closed`), 비밀번호 오류, `AbortError`.
  이것들은 버그가 아니라 정상 흐름이다.
- DSN 미설정 시 자동 no-op
...
토이 프로젝트지만 이 원칙 덕분에 **아무 키 없이 클론해서 바로 돌려볼 수 있다.**

---

## 8. UI 계층 함정

- **Tailwind v4는 `tailwind.config.js`가 없다.** `src/index.css`의 `@theme`이 토큰 소스다.
  shadcn CLI가 v4와 호환되지 않아 컴포넌트는 수동 설치했다.
- **Framer Motion + Tailwind width**: `flex items-center justify-center` 부모 안의
  `motion.div`에서 `w-full` / `max-w-sm`이 무시될 수 있다. 증상은 **텍스트가 세로로
  한 글자씩** 나오는 것. 인라인 스타일로 명시한다.
- 모든 Dialog에 `DialogTitle` 필수 (접근성).
...
프로젝트 룰은 `.claude/rules/{error-handling,design-system,data-patterns}.md`에 있다.
